	...
	if mapSystemDateTime, ok := ifaceSystemDateTime.(map[string]interface{}); ok {
		...
		if ifaceUCTDT, ok := mapSystemDateTime["UTCDateTime"].(map[string]interface{}); ok {
			...
			if mapTime, ok := time.(map[string]interface{}); ok {
				systemDT.UTCDateTime.Hour = interfaceToInt(mapTime["Hour"])
				systemDT.UTCDateTime.Minute = interfaceToInt(mapTime["Minute"])
				systemDT.UTCDateTime.Second = interfaceToInt(mapTime["Second"])
			}

			date := ifaceUCTDT["Date"]

			if mapDate, ok := date.(map[string]interface{}); ok {
				systemDT.UTCDateTime.Day = interfaceToInt(mapDate["Day"])
				systemDT.UTCDateTime.Month = interfaceToInt(mapDate["Month"])
				systemDT.UTCDateTime.Year = interfaceToInt(mapDate["Year"])
			}
		}

		if ifaceUCTDT, ok := mapSystemDateTime["LocalDateTime"].(map[string]interface{}); ok {
			time := ifaceUCTDT["Time"]
			if mapTime, ok := time.(map[string]interface{}); ok {
				systemDT.LocalDateTime.Hour = interfaceToInt(mapTime["Hour"])
				systemDT.LocalDateTime.Minute = interfaceToInt(mapTime["Minute"])
				systemDT.LocalDateTime.Second = interfaceToInt(mapTime["Second"])
			}

			date := ifaceUCTDT["Date"]

			if mapDate, ok := date.(map[string]interface{}); ok {
				systemDT.LocalDateTime.Day = interfaceToInt(mapDate["Day"])
				systemDT.LocalDateTime.Month = interfaceToInt(mapDate["Month"])
				systemDT.LocalDateTime.Year = interfaceToInt(mapDate["Year"])
			}
		}
	}
		...
		body = `<SetSystemDateAndTime xmlns="http://www.onvif.org/ver10/device/wsdl">
					...
					<UTCDateTime>
						<Time xmlns="http://www.onvif.org/ver10/schema">
							<Hour>` + intToString(systemDT.UTCDateTime.Hour) + `</Hour>
							<Minute>` + intToString(systemDT.UTCDateTime.Minute) + `</Minute>
							<Second>` + intToString(systemDT.UTCDateTime.Second) + `</Second>
						</Time>
						<Date xmlns="http://www.onvif.org/ver10/schema">
							<Year>` + intToString(systemDT.UTCDateTime.Year) + `</Year>
							<Month>` + intToString(systemDT.UTCDateTime.Month) + `</Month>
							<Day>` + intToString(systemDT.UTCDateTime.Day) + `</Day>
						</Date>
					</UTCDateTime>
